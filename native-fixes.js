...
(function(){
  ...
  function asMods(n,mode,depth=0){const t=tag(n);if(t==='hr')return[divider()];if(t==='details')return[toggle(n)];if(t==='form')return[form(n)];if(isBtn(n))return[btn(n)];if(t==='img')return[img(n)];if(['h1','h2','h3','h4','h5','h6','p','ul','ol','blockquote','span','strong'].includes(t))return[text(n.outerHTML||n.textContent||'',norm(n.textContent||'').slice(0,90))];const k=kids(n);if(k.length===1&&tag(k[0])==='img')return[img(k[0])];return mods(n,mode,depth)}
